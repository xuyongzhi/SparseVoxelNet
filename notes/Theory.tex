...
\begin{itemize}
	\item \smiley Better real meaning for labels. Commonly used for 3D viewers. Used in Matterport Dataset.
	\item Search for neighbour faces
	\item Still have to get point feature at first
	\item Naturally differentiate neighbour edges  
\end{itemize}

\newpage
\subsection{solution 1: VEFV}
...
$$ V_0 -> E_0 -> F_0 -> V_1 -> E_1 -> F_1 -> V_2$$
$$ E_{0i} = W_{e0}(V_i-V_0)+W_{e1}(\frac{V_i+V_0}{2}) \ i=(1,2)$$
$$ E_{12} = W_{e2}(|V_2-V_1|)+W_{e1}(\frac{V_1+V_2}{2})$$
$$ F = [max(E01, E02, E12), mean(E01, E02, E12)] $$
$$ V_p^{l+1} = W_0 V_p^l + max_{q\in N(p)}\{F_q\}$$

\par \noindent
The memory consumption is large.

\subsubsection{VEFV1}
Only use two edges for each face. \par 
$$ V_0(nv,3) -> E_0(nv,10,3) -> F_0(nv,10,) -> V_1 -> E_1 -> F_1 -> V_2$$
$$ E = V-V_0 $$
$$ E = W_eE $$
$$F_g= \max_{j=0,1,2} V_j$$
$$F_l= \max_{j=1,2} e_j$$
$$ F = W_f [F_l, F_g] $$
$$ V_p^{l+1} = W_{v} \max_{q\in N(p)}\{F_q\}$$


\subsection{solution 2: VFV} 
\subsubsection{VFV0}
$$F_g=W_{gf} \max_{j=0,1,2} V_j$$
$$F_l=\sum_{j=1,2}W_{lf}(V_j-V_0)$$
$$ F=[f_g, f_l] $$
$$ V_p^{l+1} = W_{v0} V_p^l + W_{v1} \max_{q\in N(p)}\{F_q\}$$
