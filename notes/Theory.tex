...
\section{Graph CNN: point, edge and face representation}
...
\subsection{solution 1: face conv}
Use point representation during feature extraction and down-sampling. Choose to predict point label or face label at last. 

\begin{tikzpicture}
\draw (0,4) node[anchor=south]{$v_2$}
-- (2,0) node[anchor=north]{$v_0$} node[midway, left] {$e_{02}$}
-- (4,4) node[anchor=south]{$v_1$} node[midway, right] {$e_{01}$}
-- cycle  node[midway, above] {$e_{12}$};
\end{tikzpicture}
